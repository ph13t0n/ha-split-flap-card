...
const SFC_THEMES = {
  ...
  classic_airport: { label: "Classic Airport", description: "Dark airport wayfinding sign style with clear yellow lettering.", bg: "#152228", top: "#213239", bottom: "#091216", line: "#05090c", border: "#d8dfe2", text: "#f3d54e", font: "Barlow Condensed, Arial Narrow, sans-serif", weight: 900 },
  arlanda_express: { label: "Arlanda Express", description: "Dark blue airport rail display with yellow lettering.", bg: "#061b3d", top: "#0b2f67", bottom: "#020d24", line: "#020716", border: "#f4cf3f", text: "#f4cf3f", font: "Barlow Condensed, Arial Narrow, sans-serif", weight: 900 },
  ...
  sweden_delight: { label: "Nordic Wayfinding", description: "Swedish blue/yellow high-contrast wayfinding style.", bg: "#005293", top: "#006aa7", bottom: "#003f73", line: "#002f56", border: "#fecb00", text: "#fecb00", font: "Sweden Sans, Noto Sans, Arial, sans-serif", weight: 900 },
  terminal_blue_orange: { label: "Terminal Blue / Orange", description: "Retro terminal signage with deep blue panels and orange information color.", bg: "#06173a", top: "#0d2b63", bottom: "#020b20", line: "#010614", border: "#f59b2e", text: "#f59b2e", font: "Barlow Condensed, Arial Narrow, sans-serif", weight: 900 },
  gate_orange: { label: "Gate Orange", description: "Orange gate-board inspired display with dark navy lettering.", bg: "#e88d2d", top: "#f5a64c", bottom: "#b96517", line: "#8a4710", border: "#06173a", text: "#071a3a", font: "Barlow Condensed, Arial Narrow, sans-serif", weight: 900 }
};

const SFC_FONT_PRESETS = {
  theme_default: { label: "Theme default", description: "Uses the selected card theme typography.", family: null, weight: null, sample: "THEME 123" },
  mechanical: { label: "Mechanical", description: "Classic split-flap / mechanical display look.", family: "Roboto Condensed, Arial Narrow, sans-serif", weight: 900, sample: "MECHANICAL" },
  transit: { label: "Transit", description: "Airport, railway station and departure board style.", family: "Barlow Condensed, Arial Narrow, sans-serif", weight: 900, sample: "ARLANDA C 12:45" },
  clean: { label: "Clean", description: "Modern dashboard-friendly UI typography.", family: "Inter, system-ui, sans-serif", weight: 800, sample: "Clean Status" },
  mono: { label: "Mono", description: "Technical terminal / monospace display style.", family: "Roboto Mono, monospace", weight: 800, sample: "MONO 08:42" },
  custom: { label: "Custom", description: "Enter your CSS font-family in Advanced styling → Font family. Example: Bitter, serif.", family: "system-ui, sans-serif", weight: 800, sample: "Custom CSS" }
};
...
class HASplitFlapCard extends HTMLElement {
  ...
  _inferSource(config) {
    return config.source || (config.entity ? "entity" : (config.clock || (config.clock_format && !config.text) ? "clock" : "text"));
  }
  ...
  _clock(format) {
    ...
    return String(format || "HH:mm:ss").replaceAll("HH", values.HH).replaceAll("hh", values.hh).replaceAll("H", values.H).replaceAll("h", values.h).replaceAll("mm", values.mm).replaceAll("ss", values.ss).replaceAll("A", values.A);
  }
  ...
  _styles() {
    ...
    return `<style>:host{display:block}ha-card{overflow:hidden;border-radius:16px;background:${this._css(c.card_background, "#030303")};padding:16px}.display-shell{position:relative;box-sizing:border-box;width:100%;border-radius:${Math.max(radius + 8, 12)}px;background:${this._css(c.frame_background, "#050505")};border:1px solid rgba(255,255,255,.10);padding:${Math.max(12, Math.round(gap * 2.25))}px;box-shadow:inset 0 0 0 1px rgba(0,0,0,.8),0 14px 30px rgba(0,0,0,.45)}.frame-none{border-color:transparent;background:transparent;padding:0;box-shadow:none}.frame-heavy{border-width:2px;padding:${Math.max(16, Math.round(gap * 3))}px}.frame-heavy:before{content:"";position:absolute;inset:7px;border:1px solid rgba(255,255,255,.07);border-radius:${Math.max(radius + 4, 10)}px;pointer-events:none}.depth-flat{box-shadow:none}.depth-raised{box-shadow:inset 0 0 0 1px rgba(0,0,0,.82),0 14px 30px rgba(0,0,0,.45)}.depth-recessed{box-shadow:inset 0 18px 32px rgba(0,0,0,.78),inset 0 0 0 1px rgba(255,255,255,.05);background:linear-gradient(180deg,#020202,#0b0b0b)}.screw{position:absolute;z-index:12;width:7px;height:7px;border-radius:50%;background:radial-gradient(circle at 35% 35%,rgba(255,255,255,.45),rgba(255,255,255,.12) 38%,rgba(0,0,0,.78) 70%);box-shadow:inset 0 0 0 1px rgba(0,0,0,.55)}.screw-tl{top:7px;left:7px}.screw-tr{top:7px;right:7px}.screw-bl{bottom:7px;left:7px}.screw-br{bottom:7px;right:7px}.display{display:flex;align-items:center;gap:${gap}px;min-width:0;overflow:hidden;perspective:1100px;transform:${c.display_depth === "recessed" ? "translateY(1px)" : "none"}}.tile{position:relative;flex:0 0 ${width}px;width:${width}px;height:${height}px;border-radius:${radius}px;overflow:hidden;background:${this._css(c.segment_background, "#101010")};border:1px solid ${this._css(c.segment_border_color, "#313131")};box-shadow:inset 0 1px 0 rgba(255,255,255,.08),inset 0 -12px 18px rgba(0,0,0,.42),0 4px 10px rgba(0,0,0,.62);transform-style:preserve-3d}.space{opacity:.82}.flap{position:absolute;left:0;right:0;height:50%;overflow:hidden;display:flex;justify-content:center;color:${this._css(c.text_color, "#ffc02e")};font-family:${this._font(c.font_family)};font-size:${fontSize}px;font-weight:${this._weight(c.font_weight)};font-style:${c.font_style === "italic" ? "italic" : "normal"};letter-spacing:${letterSpacing}px;line-height:1;backface-visibility:hidden;transform-style:preserve-3d;${glow}}.top{top:0;z-index:5;align-items:flex-end;transform-origin:bottom center;background:linear-gradient(180deg,${this._css(c.segment_background_top, "#252525")},${this._css(c.segment_background, "#101010")})}.bottom{bottom:0;z-index:4;align-items:flex-start;transform-origin:top center;background:linear-gradient(180deg,${this._css(c.segment_background, "#101010")},${this._css(c.segment_background_bottom, "#080808")})}.top span{transform:translateY(calc(50% + ${verticalOffset}px))}.bottom span{transform:translateY(calc(-50% + ${verticalOffset}px))}.hinge{position:absolute;left:0;right:0;top:calc(50% - 1px);height:2px;background:${this._css(c.segment_separator_color, "#010101")};z-index:8;box-shadow:0 -1px 0 rgba(255,255,255,.04),0 1px 0 rgba(0,0,0,.8)}.pin{position:absolute;z-index:9;top:calc(50% - 2px);width:4px;height:4px;border-radius:50%;background:rgba(255,255,255,.17)}.left{left:4px}.right{right:4px}.tile.flip .top{animation:sfcFlipTop .46s cubic-bezier(.18,.78,.22,1)}.tile.flip .bottom{animation:sfcFlipBottom .46s cubic-bezier(.18,.78,.22,1) .08s both}.tile.flip .hinge{animation:sfcHingePulse .50s ease-out}@keyframes sfcFlipTop{0%{transform:rotateX(0deg);filter:brightness(1.15);box-shadow:0 0 0 rgba(0,0,0,0)}38%{transform:rotateX(-86deg);filter:brightness(.72);box-shadow:0 11px 16px rgba(0,0,0,.55)}60%{transform:rotateX(-105deg);filter:brightness(.55)}100%{transform:rotateX(0deg);filter:brightness(1);box-shadow:0 0 0 rgba(0,0,0,0)}}@keyframes sfcFlipBottom{0%{transform:rotateX(88deg);filter:brightness(.58)}45%{transform:rotateX(18deg);filter:brightness(1.08)}100%{transform:rotateX(0deg);filter:brightness(1)}}@keyframes sfcHingePulse{0%,100%{opacity:1}50%{opacity:.55}}</style>`;
  }
...
class SplitFlapCardEditor extends HTMLElement {
  constructor() { super(); this.attachShadow({ mode: "open" }); this._config = {}; this._hass = null; this._advanced = false; this._supportOpen = false; this._issueFields = { description: "", expected: "", actual: "", console_errors: "" }; }
  set hass(hass) { this._hass = hass; this._updateSummary(); this._updateDiagnostics(); }
  setConfig(config) { this._config = { ...(config || {}) }; if (!this._config.text && !this._config.entity) this._config.text = SFC_DEFAULT_TEXT; if (!this._config.clock_format) this._config.clock_format = "HH:mm:ss"; if (!this._config.frame_style) this._config.frame_style = "classic"; if (!this._config.display_depth) this._config.display_depth = "raised"; if (this._config.frame_style === "none") this._config.decorative_screws = false; this._render(); }
  ...
  _set(key, value, render = true) { const next = { ...this._config, [key]: value }; if (value === "" && !["text", "entity", "attribute", "font_family", "font_stylesheet", "clock_format"].includes(key)) delete next[key]; if (key === "source") { if (value === "text") { delete next.entity; delete next.attribute; if (!next.text) next.text = SFC_DEFAULT_TEXT; } if (value === "entity") delete next.text; if (value === "clock") { delete next.entity; delete next.attribute; if (!next.text) next.text = SFC_DEFAULT_TEXT; if (!next.clock_format) next.clock_format = "HH:mm:ss"; if (!next.segments_mode) next.segments_mode = "auto"; } } if (key === "frame_style" && value === "none") next.decorative_screws = false; if (key === "segments_mode" && value === "auto") delete next.segments; this._config = next; this._fire(); if (render) this._render(); else { this._updateSummary(); this._updateDiagnostics(); } }
  ...
  _render() { const source = this._source(); const theme = this._v("theme", "mechanical_gold"); const preset = this._v("font_preset", "theme_default"); const segmentsMode = this._segmentsMode(); const frameStyle = this._v("frame_style", "classic"); this.shadowRoot.innerHTML = `${this._styles()}<div class="editor"><div class="hero"><div class="hero-title"><strong>Split-Flap Card</strong><span>${SPLIT_FLAP_CARD_VERSION}</span></div><div class="hint">Beta 16 editor and display polish: frame controls, depth modes, clock defaults, font previews and improved upper-flap motion.</div></div><section class="section"><div class="section-title">Content</div><div class="body"><div class="grid">${this._select("source", "Source", source, { text: "Text", entity: "Entity / Sensor", clock: "Clock" })}${this._select("segments_mode", "Segments mode", segmentsMode, { auto: "Auto — follows output length", manual: "Manual — fixed segment count" })}</div>${segmentsMode === "manual" ? this._number("segments", "Segments", this._v("segments", 16), 1, 160, "Default: 16") : this._info("Auto segments", `Current output length: ${this._effectivePreviewLength()}`)}${source === "text" ? this._input("text", "Text", this._v("text", SFC_DEFAULT_TEXT), "Text to show on the split-flap display") : ""}${source === "entity" ? this._input("entity", "Entity", this._v("entity", ""), "input_text.split_flap_message") + this._input("attribute", "Attribute", this._v("attribute", ""), "Optional attribute") : ""}${source === "clock" ? `<div class="grid">${this._input("clock_format", "Clock format", this._v("clock_format", "HH:mm:ss"), "HH:mm, HH:mm:ss, h:mm A")}${this._number("clock_tick_interval", "Clock tick interval", this._v("clock_tick_interval", 1000), 250, 60000, "Default: 1000")}</div>${this._info("Supported clock formats", "Examples: HH:mm, HH:mm:ss, h:mm A. HH/H = 24-hour, h/hh = 12-hour, mm = minutes, ss = seconds, A = AM/PM.")}` : ""}<div class="grid">${this._select("language", "Language", this._v("language", "sv"), { en: "English", sv: "Swedish", nordic: "Nordic", western: "Western" })}${this._select("charset", "Charset", this._v("charset", this._v("language", "sv")), { en: "English", sv: "Swedish", nordic: "Nordic", western: "Western", weather: "Weather", weather_sv: "Weather Swedish", extended: "Extended", custom: "Custom" })}</div></div></section><section class="section"><div class="section-title">Appearance</div><div class="body"><div class="grid">${this._select("theme", "Theme", theme, Object.fromEntries(Object.entries(SFC_THEMES).map(([key, value]) => [key, value.label])))}${this._select("align", "Align", this._v("align", "center"), { left: "Left", center: "Center", right: "Right" })}</div><div class="grid">${this._select("text_transform", "Text transform", this._v("text_transform", "uppercase"), { uppercase: "Uppercase", lowercase: "Lowercase", none: "None" })}${this._number("font_size", "Font size", this._v("font_size", ""), 6, 220, "Default: 60")}</div>${this._select("font_preset", "Font preset", preset, Object.fromEntries(Object.entries(SFC_FONT_PRESETS).map(([key, value]) => [key, value.label])))}<div class="compact-hint" data-preset-description></div>${this._fontPreview(preset, theme)}<div class="grid">${this._select("frame_style", "Display frame", frameStyle, { none: "None", classic: "Classic frame", heavy: "Heavy frame" })}${this._select("display_depth", "Split-flap layer", this._v("display_depth", "raised"), { flat: "Flat", raised: "Raised / above frame", recessed: "Recessed / behind frame" })}</div>${this._check("decorative_screws", "Decorative screws", frameStyle !== "none" && this._v("decorative_screws", true), frameStyle === "none")}${frameStyle === "none" ? this._info("Screws disabled", "Decorative screws are only available when a display frame is active.") : ""}</div></section><section class="section"><div class="section-title">Motion</div><div class="body"><div class="checks">${this._check("animation", "Animation", this._v("animation", true))}${this._check("initial_animation", "Initial animation", this._v("initial_animation", true))}${this._check("cycle_chars", "Cycle characters", this._v("cycle_chars", true))}</div></div></section><details ${this._advanced ? "open" : ""}><summary>Advanced styling</summary><div class="body"><div class="grid">${this._number("segment_width", "Segment width", this._v("segment_width", ""), 8, 180, "Default: 48")}${this._number("segment_height", "Segment height", this._v("segment_height", ""), 16, 240, "Default: 78")}</div><div class="grid">${this._number("segment_gap", "Segment gap", this._v("segment_gap", ""), 0, 80, "Default: 6")}${this._number("segment_radius", "Segment radius", this._v("segment_radius", ""), 0, 40, "Default: 7")}</div><div class="grid">${this._number("letter_spacing", "Letter spacing", this._v("letter_spacing", ""), -20, 40, "Default: -1")}${this._number("letter_vertical_offset", "Letter vertical offset", this._v("letter_vertical_offset", ""), -80, 80, "Default: -9")}</div><div class="grid">${this._input("font_family", "Font family", this._v("font_family", ""), "Example: Bitter, serif")}${this._input("text_color", "Text color", this._v("text_color", ""), "Theme default")}</div></div></details><section class="section summary-section"><div class="section-title">Preview status</div><div class="body"><div class="compact-hint" data-preview-summary></div></div></section><section class="section support-section"><div class="section-title">Support</div><div class="body"><div class="diagnostics" data-diagnostics></div><div class="links"><button type="button" data-url="${SFC_REPO_URL}/blob/main/docs/UI_EDITOR_MANUAL.md">Manual</button><button type="button" data-url="${SFC_REPO_URL}/blob/main/SUPPORT.md">Support docs</button><button type="button" data-report>Report issue</button></div></div></section></div>${this._supportOpen ? this._reportModal() : ""}`; this._bind(); this._updateSummary(); this._updateDiagnostics(); }
  _bind() { this.shadowRoot.querySelectorAll("select[data-k]").forEach((element) => { element.onchange = () => this._set(element.dataset.k, element.value, true); }); this.shadowRoot.querySelectorAll("input[data-k]").forEach((element) => { element.oninput = () => this._setDraft(element.dataset.k, element.value); element.onchange = () => this._set(element.dataset.k, element.value, false); element.onblur = () => this._set(element.dataset.k, element.value, false); }); this.shadowRoot.querySelectorAll("input[data-n]").forEach((element) => { element.oninput = () => this._numDraft(element.dataset.n, element.value); element.onchange = () => this._num(element.dataset.n, element.value, false); element.onblur = () => this._num(element.dataset.n, element.value, false); }); this.shadowRoot.querySelectorAll("input[data-b]").forEach((element) => { element.onchange = () => this._set(element.dataset.b, element.checked, false); }); this.shadowRoot.querySelectorAll("[data-url]").forEach((element) => { element.onclick = () => this._openUrl(element.dataset.url); }); this.shadowRoot.querySelector("[data-report]")?.addEventListener("click", () => { this._supportOpen = true; this._render(); }); this.shadowRoot.querySelector("[data-close-report]")?.addEventListener("click", () => { this._supportOpen = false; this._render(); }); this.shadowRoot.querySelector("[data-clear-issue]")?.addEventListener("click", () => { this._issueFields = { description: "", expected: "", actual: "", console_errors: "" }; this._render(); }); this.shadowRoot.querySelector("[data-copy-issue]")?.addEventListener("click", async () => { const text = this._issueText(); const button = this.shadowRoot.querySelector("[data-copy-issue]"); try { await navigator.clipboard?.writeText(text); if (button) button.textContent = "Copied"; } catch (error) { const output = this.shadowRoot.querySelector("[data-issue-output]"); if (output) output.value = text; if (button) button.textContent = "Select and copy"; } }); this.shadowRoot.querySelector("[data-open-issue]")?.addEventListener("click", () => { const body = encodeURIComponent(this._issueText()); const title = encodeURIComponent(`Split-Flap Card issue: ${this._source()} editor/report`); this._openUrl(`${SFC_ISSUE_URL}?title=${title}&body=${body}`); }); this.shadowRoot.querySelectorAll("[data-issue-field]").forEach((element) => { element.oninput = () => { this._issueFields[element.dataset.issueField] = element.value; const output = this.shadowRoot.querySelector("[data-issue-output]"); if (output) output.value = this._issueText(); }; }); const details = this.shadowRoot.querySelector("details"); if (details) details.ontoggle = () => { this._advanced = details.open; }; }
  _updateSummary() { const summary = this.shadowRoot?.querySelector("[data-preview-summary]"); if (summary) summary.textContent = this._previewSummary(); const presetDescription = this.shadowRoot?.querySelector("[data-preset-description]"); if (presetDescription) presetDescription.textContent = this._presetDescription(this._v("font_preset", "theme_default")); }
  _updateDiagnostics() { const diagnostics = this.shadowRoot?.querySelector("[data-diagnostics]"); if (diagnostics) diagnostics.textContent = this._diagnosticsHtml(); const output = this.shadowRoot?.querySelector("[data-issue-output]"); if (output) output.value = this._issueText(); }
  _previewSummary() { const mode = this._segmentsMode(); const segments = mode === "auto" ? `auto (${this._effectivePreviewLength()})` : String(this._v("segments", 16)); return `Source: ${this._source()} · Segments: ${segments} · Frame: ${this._v("frame_style", "classic")} · Layer: ${this._v("display_depth", "raised")}`; }
  _presetDescription(presetKey) { const preset = SFC_FONT_PRESETS[presetKey] || SFC_FONT_PRESETS.theme_default; return preset.description; }
  _fontPreview(presetKey, themeKey) { const preset = SFC_FONT_PRESETS[presetKey] || SFC_FONT_PRESETS.theme_default; const theme = SFC_THEMES[themeKey] || SFC_THEMES.mechanical_gold; const family = preset.family || theme.font; const weight = preset.weight || theme.weight; const sample = preset.sample || "SPLIT-FLAP"; return `<div class="font-preview" style="font-family:${this._font(family)};font-weight:${this._weight(weight)}"><span>${this._e(sample)}</span></div>`; }
  _effectivePreviewText() { const source = this._source(); if (source === "clock") return this._clockPreview(this._v("clock_format", "HH:mm:ss")); if (source === "entity") { const entity = this._v("entity", ""); const attribute = this._v("attribute", ""); const state = entity ? this._hass?.states?.[entity] : null; if (!entity) return ""; if (!state) return "ENTITY NOT FOUND"; return attribute ? String(state.attributes?.[attribute] ?? "") : String(state.state); } return String(this._v("text", SFC_DEFAULT_TEXT)); }
