const SPLIT_FLAP_CANARY_VERSION = "0.1.0-alpha.2-canary.6";

class HASplitFlapCardCanaryV6 extends HTMLElement {
  ...
  static getConfigElement() {
    return document.createElement("split-flap-card-canary-v6-editor");
  }
  ...
  _clearAnimationTimers() {
    this._animationTimers.forEach((id) => window.clearTimeout(id));
    this._animationTimers = [];
  }
  ...
  _themes() {
    return {
      ...
      classic_airport: {
        ...
        card_background: "#111719",
        frame_background: "#1a2225",
        segment_background: "#111719",
      ...
      terminal_amber: {
        ...
        card_background: "#f7d53b",
        frame_background: "#f7d53b",
      ...
      home_assistant_blue: {
        ...
        card_background: "#050b10",
        frame_background: "#101820",
      ...
      sweden_delight: {
        ...
        card_background: "#003f58",
        frame_background: "#004d6b",
  ...
  _fontPreset(value, theme) {
    const selected = this._themes()[theme] || this._themes().mechanical_gold;
    const presets = {
      theme_default: {
        font_family: selected.font_family || "Roboto Condensed, Arial Narrow, sans-serif",
        font_weight: selected.font_weight || 900
      },
      ...
      custom: {
        font_family: this._config?.font_family || selected.font_family || "Roboto Condensed, Arial Narrow, sans-serif",
        font_weight: this._config?.font_weight || selected.font_weight || 900
      }
    };
  ...
  _applyThemeDefaults() {
    const theme = this._themes()[this._config.theme] || this._themes().mechanical_gold;
    ...
    if (this._config.text_glow === undefined || this._config.text_glow === null || this._config.text_glow === "") {
      this._config.text_glow = "off";
    }
  ...
  _updateFromSource(force = false) {
    ...
    const allowAnimation = this._config.animation !== false && this._num(this._config.flip_duration, 0, 0, 3000) > 0;
    const allowInitial = this._config.initial_animation !== false;
    const shouldAnimate = allowAnimation && (!firstRender || allowInitial);
    ...
    this._animateTo(target, firstRender);
  }

  _animateTo(target, firstRender) {
    this._clearAnimationTimers();
    const from = firstRender ? "".padEnd(Array.from(target).length, " ") : (this._visibleText || "".padEnd(Array.from(target).length, " "));
    const cycleCount = this._config.cycle_chars === false ? 0 : this._num(this._config.cycle_count, 2, 0, 8);
  ...
  _styles(config) {
    ...
    const glow = config.text_glow === "medium"
    ...
    return `<style>
      :host{display:block;--sfc-accent:${textColor};--sfc-muted:#86754d}
      ha-card{background:${this._color(config.card_background,"#030303")};border-radius:18px;padding:18px;box-sizing:border-box;overflow:hidden;border:1px solid rgba(255,255,255,.075);box-shadow:inset 0 1px 0 rgba(255,255,255,.08),inset 0 -22px 34px rgba(0,0,0,.18),0 18px 38px rgba(0,0,0,.26)}
      ...
      .current{z-index:1}.flip-old{z-index:5}.flip-new{z-index:4;transform:rotateX(90deg);filter:brightness(.55)}.animate .flip-old{animation:sfc-v6-top ${duration}ms cubic-bezier(.2,.75,.16,1) var(--delay) both}.animate .flip-new{animation:sfc-v6-bottom ${duration}ms cubic-bezier(.2,.75,.16,1) var(--delay) both}
      ...
      @keyframes sfc-v6-top{0%{transform:rotateX(0deg);filter:brightness(1)}42%{transform:rotateX(-84deg);filter:brightness(.42)}43%,100%{transform:rotateX(-90deg);filter:brightness(.18)}}
      @keyframes sfc-v6-bottom{0%,42%{transform:rotateX(90deg);filter:brightness(.28)}55%{transform:rotateX(72deg);filter:brightness(.55)}100%{transform:rotateX(0deg);filter:brightness(1)}}
...
class SplitFlapCardCanaryV6Editor extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    this._config = {};
  ...
  set hass(hass) {
    const nextLanguage = String(hass?.locale?.language || hass?.language || "en").toLowerCase();
    const shouldRender = !this._rendered || nextLanguage !== this._language;
    this._hass = hass;
  ...
  _lang() {
    return this._language.startsWith("sv") ? "sv" : "en";
  }

  _t(key) {
    const sv = {
      content: "Innehåll", show: "Vad ska kortet visa?", text: "Text", entity: "Entitet / sensor", clock: "Klocka",
      text_to_show: "Text att visa", entity_id: "Entitet eller sensor", attribute: "Attribut, valfritt", clock_format: "Klockformat", update_interval: "Uppdateringsintervall",
      display: "Display", alignment: "Justering", flap_slots: "Teckenrutor", theme: "Tema", typography_style: "Typografistil",
      animation: "Animation", animation_on: "Aktivera animation", initial_animation: "Startanimation", animation_feel: "Animationskänsla",
      advanced: "Avancerat läge", advanced_typography: "Avancerad typografi", custom_font: "Egen font-family", font_size: "Textstorlek", font_weight: "Tjocklek", italic: "Kursiv", letter_spacing: "Teckenavstånd", letter_vertical_offset: "Bokstavsposition upp/ned", text_transform: "Textomvandling", text_glow: "Textglöd",
      ...
      colors: "Färger", card_background: "Kortbakgrund", frame_background: "Ram / housing", flap_top: "Övre flap-färg", flap_bottom: "Nedre flap-färg", flap_border: "Flap-kant", split_line: "Delningslinje", text_color: "Textfärg", advanced_data: "Avancerad textdata", charset: "Teckenuppsättning", custom_charset: "Egen teckenuppsättning", fallback_character: "Ersättningstecken", pad_character: "Utfyllnadstecken",
      manual: "Manual & hjälp", repo: "GitHub-repository", support: "Stöd projektet på Ko-fi", manual_link: "Öppna manual", help: "Hjälp", standard_hint: "Vanliga inställningar visas först. Avancerade val ligger längre ned.", animation_custom_hint: "Custom använder värdena under Finjustering av animation i avancerat läge."
    };
    const en = {
      content: "Content", show: "What should the card show?", text: "Text", entity: "Entity / Sensor", clock: "Clock",
      text_to_show: "Text to display", entity_id: "Entity or sensor", attribute: "Attribute, optional", clock_format: "Clock format", update_interval: "Update interval",
      display: "Display", alignment: "Alignment", flap_slots: "Flap slots", theme: "Theme", typography_style: "Typography style",
      animation: "Animation", animation_on: "Enable animation", initial_animation: "Initial animation", animation_feel: "Animation feel",
      advanced: "Advanced mode", advanced_typography: "Advanced typography", custom_font: "Custom font-family", font_size: "Font size", font_weight: "Font weight", italic: "Italic", letter_spacing: "Letter spacing", letter_vertical_offset: "Letter position up/down", text_transform: "Text transform", text_glow: "Text glow",
      ...
      colors: "Colors", card_background: "Card background", frame_background: "Frame / housing", flap_top: "Top flap color", flap_bottom: "Bottom flap color", flap_border: "Flap border", split_line: "Split line", text_color: "Text color", advanced_data: "Advanced text data", charset: "Charset", custom_charset: "Custom charset", fallback_character: "Fallback character", pad_character: "Pad character",
      manual: "Manual & Help", repo: "GitHub repository", support: "Support on Ko-fi", manual_link: "Open manual", help: "Help", standard_hint: "Common settings are shown first. Advanced controls are grouped below.", animation_custom_hint: "Custom uses the values under Animation fine-tuning in advanced mode."
    };
  ...
  _help(key) {
    const sv = {
      content: "Välj om kortet ska visa fast text, en Home Assistant-entitet/sensor eller en klocka.",
      display: "Teckenrutor anger hur många split-flap-rutor som visas. Detta motsvarar segments i YAML.",
      theme: "Välj färdigt tema och typografistil. Teman är valda för hög kontrast och krispig läsbarhet.",
      animation: "Animationskänsla styr bläddringstid, fördröjning och mellansteg. Custom visar finjustering i avancerat läge.",
      advanced: "Avancerat läge visar detaljerade kontroller för typografi, färger, teckenrutor, animation och teckenuppsättning.",
      manual: "Här finns länkar till dokumentation, GitHub och frivilligt projektstöd."
    };
    const en = {
      content: "Choose fixed text, a Home Assistant entity/sensor, or a browser clock.",
      display: "Flap slots define how many split-flap character slots are shown. This maps to segments in YAML.",
      theme: "Choose a theme and typography style. Themes are chosen for high contrast and crisp readability.",
      animation: "Animation feel controls flip duration, delay and shuffle steps. Custom reveals fine-tuning in advanced mode.",
      advanced: "Advanced mode reveals detailed typography, color, flap slot, animation and character controls.",
      manual: "Documentation, GitHub and optional project support links live here."
    };
    const dict = this._lang() === "sv" ? sv : en;
  ...
  _themeOptions() {
    return [
      ...
      ["classic_airport", "Classic Airport"],
      ["terminal_amber", "Terminal Amber"],
      ["nordic_light", "Nordic Light"],
      ["monochrome", "Monochrome"],
      ["home_assistant_blue", "Home Assistant Blue"],
      ["sweden_delight", "Sweden Delight"]
    ];
  }
  ...
  _fontPreset(value, theme) {
    const sweden = theme === "sweden_delight";
    return {
      theme_default: sweden
        ? { font_family: "Sweden Sans, Noto Sans, Arial, sans-serif", font_weight: 900 }
        : { font_family: "Roboto Condensed, Arial Narrow, sans-serif", font_weight: 900 },
      mechanical: { font_family: "Roboto Condensed, Arial Narrow, sans-serif", font_weight: 900 },
      transit: { font_family: "Barlow Condensed, Arial Narrow, sans-serif", font_weight: 900 },
      clean: { font_family: "Inter, system-ui, sans-serif", font_weight: 800 },
      mono: { font_family: "Roboto Mono, monospace", font_weight: 800 }
    }[value] || { font_family: "Roboto Condensed, Arial Narrow, sans-serif", font_weight: 900 };
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `${this._styles()}
      <div class="editor" data-lang="${this._lang()}">
        ...
        `)}
        ${this._section("display", this._t("display"), `
          <div class="grid two">
        ...
        `)}
        ${this._section("theme", this._t("theme"), `
          <div class="grid two">
        ...
        `)}
        ${this._section("animation", this._t("animation"), `
          <div class="grid two">
          ...
          ${this._value("animation_feel", "mechanical") === "custom" ? `<p>${this._t("animation_custom_hint")}</p>` : ""}
        `)}
        ${this._section("advanced", this._t("advanced"), `${this._checkbox("editor_advanced", this._t("advanced"), false)}`, "section-toggle")}
  ...
  _section(key, title, body, extraClass = "") {
    const isOpen = this._openHelp === key;
    return `<section class="panel ${extraClass}"><div class="panel-title"><h3>${this._escape(title)}</h3><button class="help" type="button" data-help="${this._escape(key)}" aria-expanded="${isOpen ? "true" : "false"}" aria-label="${this._escape(this._t("help"))}">?</button></div>${isOpen ? `<div class="help-text">${this._escape(this._help(key))}</div>` : ""}<div class="panel-body">${body}</div></section>`;
  }
  ...
  _styles() {
    return `<style>
      :host{display:block;color:#f4f0e6;--accent:#ffc02e;--surface:#111;--surface-2:#171717;--border:#3a3325;--muted:#b8aa82;font-family:var(--primary-font-family, system-ui, sans-serif)}
      .editor{display:grid;gap:12px;background:linear-gradient(180deg,#111,#070707);border:1px solid #2c271b;border-radius:18px;padding:14px;box-sizing:border-box}.panel{background:linear-gradient(180deg,var(--surface-2),var(--surface));border:1px solid var(--border);border-radius:14px;overflow:hidden;box-shadow:0 8px 20px rgba(0,0,0,.24)}
      .panel-title{display:flex;align-items:center;justify-content:space-between;gap:8px;background:linear-gradient(90deg,rgba(255,192,46,.22),rgba(134,117,77,.08));border-bottom:1px solid rgba(255,192,46,.18);padding:9px 11px}.panel-title h3{font-size:13px;line-height:1.2;margin:0;text-transform:uppercase;letter-spacing:.06em;color:#ffe3a0}.help{display:inline-grid;place-items:center;min-width:28px;height:28px;border:0;border-radius:999px;color:#060606;background:var(--accent);font:inherit;font-weight:800}.help:focus-visible,input:focus-visible,select:focus-visible,a:focus-visible{outline:3px solid #fff;outline-offset:2px}.help-text{padding:10px 12px;border-bottom:1px solid rgba(255,192,46,.14);background:#090806;color:#ffe3a0;font-size:12px;line-height:1.4}.panel-body{padding:12px;display:grid;gap:11px}.panel-body p{margin:0;color:var(--muted);font-size:12px;line-height:1.35}
      .grid{display:grid;gap:11px}.grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}.field{display:grid;gap:5px}.field span,.check span{font-size:12px;color:#f0e6ca}.field input,.field select{width:100%;min-height:42px;box-sizing:border-box;border-radius:10px;border:1px solid #4c4129;background:#070707;color:#fff;padding:9px 10px;font:inherit}.check{display:flex;align-items:center;gap:10px;min-height:42px}.check input{inline-size:20px;block-size:20px;accent-color:var(--accent)}.link-row a{color:#ffe3a0;text-decoration:none}.link-row a:hover{text-decoration:underline}
      @media(max-width:620px){.editor{padding:10px;border-radius:14px}.grid.two{grid-template-columns:1fr}.panel-title{padding:8px 9px}.panel-body{padding:10px}.field input,.field select{min-height:44px}}
    </style>`;
  }
...
if (!customElements.get("split-flap-card-canary-v6")) {
  customElements.define("split-flap-card-canary-v6", HASplitFlapCardCanaryV6);
}

if (!customElements.get("split-flap-card-canary-v6-editor")) {
  customElements.define("split-flap-card-canary-v6-editor", SplitFlapCardCanaryV6Editor);
}

window.customCards = window.customCards || [];
window.customCards.push({
  type: "split-flap-card-canary-v6",
  name: "Split-Flap Card Canary V6",
  description: `Standalone release candidate canary (${SPLIT_FLAP_CANARY_VERSION})`
});
