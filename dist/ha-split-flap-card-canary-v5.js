const SPLIT_FLAP_CANARY_VERSION = "0.1.0-alpha.2-canary.5";

class HASplitFlapCardCanaryV5 extends HTMLElement {
  ...
  static getConfigElement() {
    return document.createElement("split-flap-card-canary-v5-editor");
  }
  ...
  _styles(config) {
    ...
    return `<style>
      ...
      .current{z-index:1}.flip-old{z-index:5}.flip-new{z-index:4;transform:rotateX(90deg);filter:brightness(.55)}.animate .flip-old{animation:sfc-v5-top ${duration}ms cubic-bezier(.2,.75,.16,1) var(--delay) both}.animate .flip-new{animation:sfc-v5-bottom ${duration}ms cubic-bezier(.2,.75,.16,1) var(--delay) both}
      ...
      @keyframes sfc-v5-top{0%{transform:rotateX(0deg);filter:brightness(1)}42%{transform:rotateX(-84deg);filter:brightness(.42)}43%,100%{transform:rotateX(-90deg);filter:brightness(.18)}}
      @keyframes sfc-v5-bottom{0%,42%{transform:rotateX(90deg);filter:brightness(.28)}55%{transform:rotateX(72deg);filter:brightness(.55)}100%{transform:rotateX(0deg);filter:brightness(1)}}
...
class SplitFlapCardCanaryV5Editor extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    this._config = {};
  ...
  _t(key) {
    const sv = {
      ...
      animation_fine: "Finjustering av animation", flip_speed: "Bläddringshastighet", delay_between: "Fördröjning mellan teckenrutor", shuffle_steps: "Mellansteg före slutbokstav",
      colors: "Färger", card_background: "Kortbakgrund", text_color: "Textfärg", advanced_data: "Avancerad textdata", charset: "Teckenuppsättning", custom_charset: "Egen teckenuppsättning", fallback_character: "Ersättningstecken", pad_character: "Utfyllnadstecken",
    ...
    const en = {
      ...
      animation_fine: "Animation fine-tuning", flip_speed: "Flip speed", delay_between: "Delay between flaps", shuffle_steps: "Shuffle steps before final letter",
      colors: "Colors", card_background: "Card background", text_color: "Text color", advanced_data: "Advanced text data", charset: "Charset", custom_charset: "Custom charset", fallback_character: "Fallback character", pad_character: "Pad character",
  ...
  _help(key) {
    const sv = {
      ...
      theme: "Välj färdigt tema och typografistil. Inga externa fonter laddas automatiskt.",
      animation: "Animationskänsla styr hastighet, fördröjning och antal mellansteg med enkla förval. Custom visar finjustering i avancerat läge.",
    ...
    const en = {
      ...
      theme: "Choose a visual theme and typography style. No external fonts are loaded automatically.",
      animation: "Animation feel controls speed, delay and shuffle steps through simple presets. Custom reveals fine-tuning in advanced mode.",
  ...
  _set(key, value, type = "string") {
    ...
    if (key === "animation_feel") {
      ...
      if (value === "custom") config.editor_advanced = true;
    }
...
if (!customElements.get("split-flap-card-canary-v5")) {
  customElements.define("split-flap-card-canary-v5", HASplitFlapCardCanaryV5);
}

if (!customElements.get("split-flap-card-canary-v5-editor")) {
  customElements.define("split-flap-card-canary-v5-editor", SplitFlapCardCanaryV5Editor);
}

window.customCards = window.customCards || [];
window.customCards.push({
  type: "split-flap-card-canary-v5",
  name: "Split-Flap Card Canary V5",
  description: `Standalone readable mechanical editor canary (${SPLIT_FLAP_CANARY_VERSION})`
});
