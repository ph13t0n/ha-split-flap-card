const SPLIT_FLAP_CANARY_VERSION = "0.1.0-alpha.2-canary.2";

class HASplitFlapCardCanaryV2 extends HTMLElement {
  ...
  _animationPreset(value) {
    const presets = {
      ...
      custom: {
        flip_duration: this._config?.flip_duration || 640,
        flip_stagger: this._config?.flip_stagger || 35,
  ...
  _styles(config) {
    ...
    const textColor = this._color(config.text_color, "#f6b72a");
    const glow = config.text_glow === "off"
      ? "none"
      : config.text_glow === "medium"
        ? `0 1px 0 rgba(0,0,0,.95), 0 0 8px ${this._rgba(textColor, .32)}`
        : `0 1px 0 rgba(0,0,0,.95), 0 0 5px ${this._rgba(textColor, .18)}`;
    const mobileWidth = Math.max(24, Math.floor(width * .74));
    const mobileHeight = Math.max(38, Math.floor(height * .74));
    const mobileFont = Math.max(20, Math.floor(fontSize * .72));
    ...
    return `<style>
      ...
      .display-shell{border-radius:12px;padding:10px;background:linear-gradient(180deg,#181818 0%,#050505 56%,#000 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,.07),inset 0 0 0 1px rgba(255,255,255,.045),inset 0 -18px 28px rgba(0,0,0,.62);overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;scrollbar-width:thin}
      .display{display:flex;align-items:center;gap:${gap}px;width:max-content;min-width:100%;box-sizing:border-box;padding:2px}
      .split-flap-tile{position:relative;width:${width}px;height:${height}px;min-width:${width}px;border-radius:${radius}px;background:${this._color(config.segment_background,"#101010")};border:1px solid ${this._color(config.segment_border_color,"#292929")};color:${textColor};font-family:${this._fontFamily(config.font_family,"Roboto Condensed, Arial Narrow, sans-serif")};font-size:${fontSize}px;font-weight:${weight};font-style:${fontStyle};letter-spacing:${letterSpacing}px;text-align:center;overflow:hidden;box-sizing:border-box;box-shadow:inset 0 1px 0 rgba(255,255,255,.115),inset 0 -1px 0 rgba(0,0,0,.9),0 9px 16px rgba(0,0,0,.62)}
      ...
      .half span{position:absolute;left:0;width:100%;height:${height}px;display:flex;align-items:center;justify-content:center;font-family:inherit;font-size:inherit;font-weight:inherit;font-style:inherit;letter-spacing:inherit;line-height:1;color:${textColor};text-shadow:${glow};transform:scaleX(.84);transform-origin:center center}.top span{top:0}.bottom span{top:-${Math.floor(height / 2)}px}
      .hinge{position:absolute;left:-1px;right:-1px;top:calc(50% - 2.5px);height:5px;background:linear-gradient(180deg,rgba(255,255,255,.11),transparent 34%),${this._color(config.segment_separator_color,"#020202")};z-index:2;box-shadow:0 1px 0 rgba(255,255,255,.075),0 -1px 0 rgba(0,0,0,.88),0 2px 5px rgba(0,0,0,.55)}
      ...
      @media(max-width:560px){ha-card{padding:12px}.display-shell{padding:7px}.display{gap:${mobileGap}px}.split-flap-tile{width:${mobileWidth}px;min-width:${mobileWidth}px;height:${mobileHeight}px;font-size:${mobileFont}px}.half span{height:${mobileHeight}px}.bottom span{top:-${Math.floor(mobileHeight / 2)}px}}
    </style>`;
  }
...
class SplitFlapCardCanaryV2Editor extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    this._config = {};
  ...
  _t(key) {
    const sv = {
      ...
      advanced: "Avancerat läge", advanced_typography: "Avancerad typografi", custom_font: "Egen font-family", font_size: "Textstorlek", font_weight: "Tjocklek", italic: "Kursiv", letter_spacing: "Teckenavstånd", text_transform: "Textomvandling", text_glow: "Textglöd",
      ...
      manual: "Manual & hjälp", repo: "GitHub-repository", support: "Stöd projektet på Ko-fi", manual_link: "Öppna manual", help: "Hjälp", standard_hint: "Vanliga inställningar visas först. Avancerade val ligger längre ned."
    };
    const en = {
      ...
      advanced: "Advanced mode", advanced_typography: "Advanced typography", custom_font: "Custom font-family", font_size: "Font size", font_weight: "Font weight", italic: "Italic", letter_spacing: "Letter spacing", text_transform: "Text transform", text_glow: "Text glow",
      ...
      manual: "Manual & Help", repo: "GitHub repository", support: "Support on Ko-fi", manual_link: "Open manual", help: "Help", standard_hint: "Common settings are shown first. Advanced controls are grouped below."
    };
  ...
  _help(key) {
    const sv = {
      ...
      animation: "Animationskänsla styr hastighet, fördröjning och antal mellansteg med enkla förval.",
      advanced: "Avancerat läge visar detaljerade kontroller för typografi, färger, teckenrutor och teckenuppsättning.",
    ...
    const en = {
      ...
      animation: "Animation feel controls speed, delay and shuffle steps through simple presets.",
      advanced: "Advanced mode reveals detailed typography, color, flap slot and character controls.",
...
if (!customElements.get("split-flap-card-canary-v2")) {
  customElements.define("split-flap-card-canary-v2", HASplitFlapCardCanaryV2);
}

if (!customElements.get("split-flap-card-canary-v2-editor")) {
  customElements.define("split-flap-card-canary-v2-editor", SplitFlapCardCanaryV2Editor);
}

window.customCards = window.customCards || [];
window.customCards.push({
  type: "split-flap-card-canary-v2",
  name: "Split-Flap Card Canary V2",
